...
public class PerlinTerrain : MonoBehaviour
{
    ...
    void Start()
    {
        if (randomizeOffset)
        {
            ...
            plantsOffsetX = Random.Range(0f, 1000f);
            plantsOffsetY = Random.Range(0f, 1000f);
        }
    ...
    void SpawnBush()
    {
        ...
        int width = terrainData.alphamapWidth;
        int height = terrainData.alphamapHeight;
        var bushPositions = new System.Collections.Generic.List<Vector3>();
        int spawnedBush = 0;
        int targetLayer = 1;
        
        for (int x = 0; x < width && spawnedBush < bushCount; x++)
        {
            for (int y = 0; y < height && spawnedBush < bushCount; y++)
            {
                Vector3 worldPos = new Vector3(
                    x * terrainData.size.x / width,
                    0,
                ...
                if (IsGrass(worldPos))
                {
                    float noise = Mathf.PerlinNoise((x + plantsOffsetX) / scale , (y + plantsOffsetY) / scale);
                    if (noise > noiseThreshold)
                    {
                        bool tooClose = false;
